...
assert.equal(document.querySelector('.gd-game-nav'), null, 'Game/Studio/Stream navigation belongs to the bottom footer panels, not inside the Game page');
assert.ok(body().includes('DLC'),'standalone DLC section is visible above the four detail tabs');
assert.ok(body().includes('Northern Reach'),'published DLC appears without opening Expansions & Cards');
...
await run(() => dlcRow.click());
await run(() => button('Add DLC to cart').click());
assert.equal(calls.cart[1].id, 'expansion');
assert.equal(calls.cart[1].gameId, game.id);
await run(() => button('Expansions & Cards').click());
assert.ok(body().includes('Stoneguard'));
assert.ok(document.querySelector('.gd-expansions-divider'),'Expansions & Cards keeps the vertical split');
assert.ok(document.querySelector('.gd-expansions-viewer'),'3D viewer occupies the dedicated side of the split');
await run(() => button('Expansions & Cards').dispatchEvent(new window.KeyboardEvent('keydown', { key: 'Home', bubbles: true })));
...
console.log('PASS: restored footer-owned Game/Studio/Stream navigation, standalone real DLC, Expansions & Cards split viewer, gallery/theater controls, cart/auth states, requirements, reviews, and route recovery.');
