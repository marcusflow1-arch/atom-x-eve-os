...
import { X, Mountain, Box, Sparkles, Swords, Film, UserRound, Camera, Layers3, Save, Sun, Users, Shield, Plus, Trash2, Upload, Play, Square } from 'lucide-react';
...
const DAMAGE_KEY = 'atomxe_editor_damage_profiles_v2';
const ACTOR_KEY = 'atomxe_editor_actor_stats_v2';
const EQUIPMENT_KEY = 'atomxe_editor_equipment_v2';
const SET_KEY = 'atomxe_editor_set_bonuses_v2';
const ANIM_KEY = 'atomxe_editor_animation_v2';
const WORLD_KEY = 'atomxe_preview_world_edits_v2';
...
export default function GameWorldEditDock({ onClose }) {
  ...
  useEffect(() => {
    if (!open || !scene || !camera || !selectedTerrain || tab !== 'world') return;
  ...
  const loadAsset=async asset=>{if(!scene||!asset)return;try{let root;if(asset.type==='gltf'||asset.type==='glb')root=(await new GLTFLoader().loadAsync(asset.url)).scene;else if(asset.type==='fbx')root=await new FBXLoader().loadAsync(asset.url);else root=await new OBJLoader().loadAsync(asset.url);root.name=asset.name.replace(/\.[^.]+$/,'');root.userData.editorAsset=true;ensureConfig(root);root.position.set(0,0,0);scene.add(root);selectObject(root);}catch(e){console.error('[GameWorldEditDock] asset load failed',e);}};
  ...
  return <aside className="pointer-events-auto fixed inset-y-0 right-0 z-[120] flex w-[20vw] min-w-[330px] max-w-[460px] flex-col border-l border-white/10 bg-slate-950/76 shadow-2xl backdrop-blur-3xl">
