...
export default function GameWorldEditController() {
  ...
  const addAssetToWorld = (asset, point = new THREE.Vector3(0, 1, 0)) => {
    ...
    if (/\.fbx$/i.test(asset.name)) new FBXLoader().load(asset.url, place, undefined, console.error); else new GLTFLoader().load(asset.url, (g) => place(g.scene || g.scenes?.[0]), undefined, console.error);
  };
  useEffect(() => {
    if (!open) return undefined;
