...
export default function SystemUpdatesRemindersOverlay({ mode = 'updates', onClose }) {
  ...
  return <AnimatePresence>
    <motion.div
      ...
      className="fixed z-[9999] left-[420px] right-0 top-[164px] bottom-[48px] overflow-hidden pointer-events-auto"
      style={{
        background: 'rgba(7,11,17,.94)',
        backdropFilter: 'blur(34px) saturate(145%)',
        WebkitBackdropFilter: 'blur(34px) saturate(145%)',
        boxShadow: '-30px 0 80px rgba(0,0,0,.42)',
        isolation: 'isolate',
      }}
      role="dialog"
      aria-modal="true"
      ...
      <div className="h-full flex flex-col">
        <header className="h-[68px] shrink-0 flex items-center justify-between px-7">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="text-white/85 text-sm font-semibold tracking-wider uppercase">{mode === 'reminders' ? 'Reminders' : 'System Updates'}</div>
              <div className="text-white/25 text-[9px] uppercase tracking-[.18em]">Luna Dashboard</div>
            </div>
          </div>
        ...
        <div className="flex-1 min-h-0 flex">
            ...
            {Object.entries(grouped).map(([date, group]) => {
              ...
              return <div key={date} className="mb-5">
                <button onClick={() => setExpanded(v => ({ ...v, [date]: !isExpanded }))} className="w-full flex items-center justify-between py-2 text-left text-white/40 hover:text-white/70 transition-colors">
                  <span className="flex items-center gap-2 text-[9px] uppercase tracking-[.18em]"><CalendarDays className="w-3 h-3"/>{date}</span>
                ...
                <AnimatePresence initial={false}>{isExpanded && <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-1 space-y-0.5 overflow-hidden">
                  {group.map(item => <motion.button layout key={item.id} onClick={() => setSelected(item)} className={`w-full text-left py-3 px-1 flex gap-3 transition-colors ${selected?.id === item.id ? 'text-white bg-white/[0.035]' : 'text-white/50 hover:text-white/80 hover:bg-white/[0.018]'}`}>
                    ...
                    <span className="min-w-0 flex-1"><span className="block text-[11px] font-medium truncate">{item.title}</span><span className="block text-[9px] text-white/25 truncate mt-1">{item.description || item.detail}</span></span>
                    <ChevronRight className="w-3 h-3 mt-1 text-white/15"/>
                  </motion.button>)}
          ...
          <section className="flex-1 min-w-0 overflow-y-auto px-9 py-7" style={{ scrollbarWidth: 'none' }}>
            <AnimatePresence mode="wait">
              {selected ? <motion.div key={selected.id} initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} className="max-w-3xl">
              ...
              </motion.div> : <div className="h-full flex items-center justify-center text-white/20 text-xs">Select an item</div>}
            </AnimatePresence>
          </section>
        </div>
      </div>
