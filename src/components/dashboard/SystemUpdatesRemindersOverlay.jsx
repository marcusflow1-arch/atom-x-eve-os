...
export default function SystemUpdatesRemindersOverlay({ mode = 'updates', onClose }) {
  ...
  return createPortal(<AnimatePresence><motion.div initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} className="fixed inset-0 z-[190000] bg-black/65 backdrop-blur-2xl pointer-events-auto" onClick={onClose}><motion.div initial={{x:'100%'}} animate={{x:0}} exit={{x:'100%'}} transition={{duration:.3,ease:[.22,1,.36,1]}} onClick={(e) => e.stopPropagation()} className="absolute bottom-0 right-0 top-0 flex w-full max-w-[780px] flex-col border-l border-white/[0.06] bg-[#060a10]/98 shadow-[-30px_0_90px_rgba(0,0,0,.6)]">
