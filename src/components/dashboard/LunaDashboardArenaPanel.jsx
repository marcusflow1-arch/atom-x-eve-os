  ...
  return (
    <div className="pointer-events-none flex h-full items-end justify-center p-5">
      <section className="pointer-events-auto w-full max-w-[1040px] overflow-hidden border border-white/[0.09] bg-[#071019]/88 shadow-[0_30px_90px_rgba(0,0,0,.48)] backdrop-blur-2xl">
        ...
        {error && <div className="border-b border-rose-100/10 bg-rose-100/[0.04] px-4 py-2 text-[10px] text-rose-100/60">{error}</div>}
        ...
        {e.status === 'lobby' ? (
          ...
          <div className="p-4">
            <div className="flex items-center justify-between"><div><p className="text-[12px] font-black uppercase tracking-[0.13em] text-rose-100/34">Incoming Attack</p><h4 className="mt-1 text-[12px] font-semibold text-white/96">{mine ? 'Read the strike. Time your defense.' : (target?.name || 'Player') + ' is defending.'}</h4></div><span className="text-[10px] text-white/96">{e.enemy?.intent}</span></div>
