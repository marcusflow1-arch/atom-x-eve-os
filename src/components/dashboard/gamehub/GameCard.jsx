...
const GameCard = ({ game, onSelect, onDoubleClick }) => {
    ...
    const handleSelect = (event) => {
        window.dispatchEvent(new CustomEvent('libraryGameSelected', { detail: { game } }));
        onSelect?.(event);
    };
    ...
    return (
        ...
        >
            ...
            <div className="p-3">
                ...
                <p className="text-slate-400 text-xs truncate">{genres.length > 0 ? genres.join(', ') : 'Uncategorized'}</p>
            </div>
        </div>
    );
};
