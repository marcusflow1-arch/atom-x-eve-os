...
export default function LibraryLandingPage({ game: selectedGame, onClose, initialTab = 'Overview' }) {
  ...
  return <div data-testid="library-full-view" className="library-surface relative h-full min-h-0 flex flex-col overflow-hidden bg-background/90 text-foreground">
    <header className="relative shrink-0 min-h-48 flex items-end p-6">
      ...
      <div className="relative flex flex-wrap w-full items-end gap-4">
        <div className="flex-1 min-w-0"><p className="text-xs text-primary">{game.genre}{game.demo ? ' · Demo catalog' : ''}</p><h1 className="mt-2 text-2xl font-bold break-words">{game.title}</h1><p className="mt-2 text-xs text-muted-foreground">Select games from the library on the left.</p></div>
