...
export default function GameWorkspace({ game, clan, onBack, initialZone }) {
  ...
  return (
    <div className="relative flex h-full w-full overflow-hidden bg-[#171c22] text-white">
      ...
      <aside className="relative z-10 flex w-[286px] shrink-0 flex-col border-r border-white/[0.07] bg-[#1d232b]/82 backdrop-blur-3xl">
        <div className="border-b border-white/[0.07] p-4">
          ...
          <div className="mt-4 flex items-center gap-3">
            <div className="grid h-11 w-11 shrink-0 place-items-center overflow-hidden rounded-xl border border-white/[0.07] bg-white/[0.04]">{gameArt ? <img src={gameArt} alt="" className="h-full w-full object-cover" /> : <Gamepad2 className="h-4 w-4 text-white/28" />}</div>
            <div className="min-w-0"><h2 className="truncate text-sm font-semibold text-white/88">{game?.title || 'Game'}</h2><div className="mt-1 flex items-center gap-2 text-[9px] text-white/30"><span className="h-1.5 w-1.5 rounded-full bg-emerald-300/70" />{activeMemberIds.length} active in workspace</div></div>
          </div>
        </div>

        {!game?.isGlobalChat && <section className="max-h-[220px] overflow-y-auto border-b border-white/[0.07] p-3">
          <div className="mb-2 flex items-center gap-2 px-1 text-[8px] font-bold uppercase tracking-[0.18em] text-white/28"><Target className="h-3 w-3" /> Active tasks</div>
          {objectives.length ? <div className="space-y-1.5">{objectives.map((objective) => <div key={objective.id} className="rounded-xl border border-white/[0.045] bg-white/[0.025] p-2.5"><div className="flex gap-2"><div className="min-w-0 flex-1"><strong className="block truncate text-[10px] font-medium text-white/70">{objective.targetName || objective.title || 'Clan task'}</strong><p className="mt-1 line-clamp-2 text-[9px] leading-4 text-white/28">{objective.notes || objective.type || 'Objective'}</p></div><button type="button" onClick={() => completeObjective.mutate(objective.id)} className="self-start rounded-md bg-white/[0.04] px-1.5 py-1 text-[8px] text-white/35 hover:bg-white/[0.08] hover:text-emerald-200/70">Done</button></div></div>)}</div> : <p className="px-1 py-3 text-[9px] text-white/22">No active tasks for this game.</p>}
        </section>}

        <nav className="min-h-0 flex-1 overflow-y-auto p-3">
          <div className="mb-2 px-1 text-[8px] font-bold uppercase tracking-[0.18em] text-white/25">Operational zones</div>
          <div className="space-y-1">{visibleZones.map((zone) => { const Icon = zone.icon; const selected = zone.id === activeZone; return <button key={zone.id} type="button" onClick={() => handleZoneChange(zone.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left transition ${selected ? 'bg-white/[0.075] text-white' : 'text-white/42 hover:bg-white/[0.035] hover:text-white/72'}`}><div className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg ${selected ? 'bg-cyan-100/[0.09] text-cyan-100/70' : 'bg-white/[0.035] text-white/32'}`}><Icon className="h-3.5 w-3.5" /></div><div className="min-w-0"><strong className="block truncate text-[10px] font-semibold">{zone.label}</strong><span className="mt-0.5 block truncate text-[8px] text-white/25">{zone.desc}</span></div></button>; })}</div>
        </nav>
      </aside>

      <main className="relative z-10 flex min-w-0 flex-1 flex-col">
        <header className="flex h-14 shrink-0 items-center gap-3 border-b border-white/[0.07] bg-black/[0.08] px-5 backdrop-blur-xl">
          <ActiveIcon className="h-4 w-4 text-cyan-100/50" />
          <div><h3 className="text-sm font-semibold text-white/84">{activeDefinition.label}</h3><p className="text-[9px] text-white/28">{activeDefinition.desc}</p></div>
          <div className="ml-auto flex items-center gap-2"><Badge variant="outline" className="border-white/[0.07] bg-white/[0.025] text-[9px] text-white/38">{activeMemberIds.length} Active</Badge><Button size="sm" variant="outline" onClick={() => setShowInvite(true)} className="h-8 gap-1.5 border-white/[0.08] bg-white/[0.025] text-[9px] text-white/50 hover:bg-white/[0.06] hover:text-white"><UserPlus className="h-3 w-3" />Invite</Button></div>
        </header>

        <div className="min-h-0 flex-1 overflow-hidden">
          {visitedZones.chat && <div className="h-full" style={{ display: activeZone === 'chat' ? 'block' : 'none' }}><ChatZone game={game} clan={clan} user={user} /></div>}
          ...
          {visitedZones.exploration && <div className="h-full" style={{ display: activeZone === 'exploration' ? 'block' : 'none' }}><ExplorationZone game={game} clan={clan} /></div>}
          ...
          {visitedZones.forms && <div className="h-full" style={{ display: activeZone === 'forms' ? 'block' : 'none' }}><ClanFormsZone game={game} clan={clan} user={user} /></div>}
          ...
          {visitedZones.party && <div className="h-full" style={{ display: activeZone === 'party' ? 'flex' : 'none' }}><div className="min-w-0 flex-1"><PartyManager clanId={clan?.id} gameId={game?.id} /></div><div className="w-72 shrink-0 border-l border-white/[0.055]"><ZoneChatPanel clanId={clan?.id} gameId={game?.id} zoneId="party" title="Party Chat" /></div></div>}
        </div>
      </main>
