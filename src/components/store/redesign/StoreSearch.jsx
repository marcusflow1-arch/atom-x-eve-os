...
export default function StoreSearch({games=[],value='',onChange,onSelect,onOpen,onSubmit}){
 ...
 return <div ref={root} className="relative min-w-0 w-full" onBlur={e=>{if(!root.current?.contains(e.relatedTarget))setOpen(false);}}>
  ...
  {open&&items.length>0&&<ul id="store-search-suggestions" role="listbox" className="absolute inset-x-0 top-full z-[150] mt-2 overflow-hidden rounded-xl bg-slate-950 p-1 shadow-2xl ring-1 ring-white/15">
   {items.map((item,i)=><li key={item.kind+item.id} id={'store-suggestion-'+i} role="option" aria-selected={active===i}>
