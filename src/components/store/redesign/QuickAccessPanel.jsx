        ...
        {QUICK_ACCESS.map(item => {
          ...
          return (
            <button key={item.id} onClick={() => onSelect?.(item.id)}
              className="group relative flex items-center gap-3 px-3 py-2.5 rounded-xl border border-white/[0.08] bg-white/[0.03] hover:bg-white/[0.07] transition-all text-left flex-1 overflow-hidden"
              ...
              onMouseLeave={(e) => { e.currentTarget.style.borderColor = ''; e.currentTarget.style.boxShadow = 'inset 0 1px 0 rgba(255,255,255,0.06)'; }}
            >
              <div className="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0"
                style={{ background: `${item.color}24`, border: `1px solid ${item.color}55`, boxShadow: `0 0 12px ${item.color}40` }}>
