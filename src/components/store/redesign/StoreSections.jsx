      ...
      {games.length ? (
        <div className="divide-y divide-white/[0.06]">
          {games.slice(0,4).map((game,i)=>(
            <button key={game.id} onClick={()=>onSelect(game.id)} className="group flex w-full items-center gap-4 rounded-lg py-3 text-left hover:bg-white/[0.03]">
        ...
        <p className="rounded-xl bg-white/[0.025] px-4 py-10 text-sm text-white/40">{empty}</p>
      )}
    </section>
  );
}
