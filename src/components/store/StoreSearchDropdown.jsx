...
export default function StoreSearchDropdown({ games, onGameSelect, isListening, toggleVoice }) {
  ...
  useEffect(() => {
    if ((isOpen || isFocused) && triggerRef.current) {
  ...
  return (
    <div className="relative flex-1 max-w-sm" ref={triggerRef}>
      ...
      {/* Backdrop + Dropdown — rendered via portal to escape stacking contexts */}
      {createPortal(
        <AnimatePresence>
          {(isOpen || isFocused) && (
            <>
              ...
              <motion.div
                ...
                style={{ zIndex: 999998, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(4px)' }}
                onClick={close}
              />
