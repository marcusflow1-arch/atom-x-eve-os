...
import { ArrowLeft, MessageSquare, Plus, Sprout } from 'lucide-react';
...
export default function FarmGameView({ game, onBack }) {
  ...
  return <div className="flex h-full flex-col overflow-hidden bg-[#020617] text-white">
    ...
    <CreatePostModal open={showCreatePostModal} onClose={() => setShowCreatePostModal(false)} topic={activeTopic || 'farming'} gameTitle={game?.title} gameId={game?.id} onCreated={() => setSearchParams((previous) => { const next = new URLSearchParams(previous); next.set('refresh', String(Date.now())); return next; })} />
  </div>;
}
