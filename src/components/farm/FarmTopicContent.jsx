...
export default function FarmTopicContent({ topic, gameId, gameTitle, isOwned, onJoinRoomRequest, intent }) {
  ...
  return (
    <div className="h-full flex">
      ...
      {/* CENTER: Posts */}
      <div className="w-[360px] flex-shrink-0 flex flex-col h-full border-r border-white/5 bg-white/[0.02]">
        <div className="px-4 pt-4 pb-3 border-b border-white/5 flex items-center justify-between">
          <div>
            <h3 className="text-xs font-bold text-white/70 uppercase tracking-wider">Farm Posts</h3>
          ...
          {posts.length === 0 ? (
            ...
            posts.map((post) => (
              <button
                ...
                className={`w-full text-left rounded-2xl border p-4 transition-all ${selectedPost?.id === post.id ? 'bg-cyan-500/10 border-cyan-500/30' : 'bg-white/[0.03] border-white/5 hover:bg-white/[0.06] hover:border-white/10'}`}
              >
            ...
            <div className="rounded-3xl border border-white/10 bg-white/[0.03] p-6">
              <div className="flex items-start justify-between gap-4 mb-5">
                ...
                <div className="flex items-center gap-2">
                  <button onClick={() => handlePostVote(selectedPost, 'up')} className="px-3 py-2 rounded-xl bg-green-500/10 text-green-400 border border-green-500/20 text-xs font-semibold">Like</button>
                  <button onClick={() => handlePostVote(selectedPost, 'down')} className="px-3 py-2 rounded-xl bg-red-500/10 text-red-400 border border-red-500/20 text-xs font-semibold">Dislike</button>
                </div>
              </div>

              <div className="text-sm text-white/75 leading-7 whitespace-pre-wrap mb-8">{selectedPost.content}</div>

              <CommentSection
                postId={selectedPost.id}
                comments={sortedComments.map((comment) => ({ ...comment, score: Number(comment.score || 0) }))}
              ...
              {sortedComments.length > 0 && (
                <div className="mt-4 space-y-2">
                  <h4 className="text-xs font-bold uppercase tracking-wider text-white/35">Boost comments</h4>
