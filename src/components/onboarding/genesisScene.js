...
export function createGenesisScene(container, url, onReady, onStatus) {
  ...
  let disposed=false,model,mixer,action,frame,appearance={},animationVersion=0;
  ...
  async function play(motion) {
    ...
    try {
      ...
      const next=mixer.clipAction(clip);action?.fadeOut(.25);next.reset().fadeIn(.25).play();action=next;model.visible=true;onStatus('ready',motion.name);disposeModel(asset);
  ...
  (async()=>{
    try {
      ...
      box=new THREE.Box3().setFromObject(model);const center=box.getCenter(new THREE.Vector3());model.position.set(-center.x,-box.min.y,-center.z);
  ...
  return {appearance:value=>{appearance=value;if(model)applyCompanionAppearance(model,value);},play,rotate:amount=>{if(model)model.rotation.y+=amount;},dispose:()=>{disposed=true;cancelAnimationFrame(frame);observer.disconnect();controls.dispose();mixer?.stopAllAction();disposeModel(model);renderer.dispose();renderer.domElement.remove();}};
}