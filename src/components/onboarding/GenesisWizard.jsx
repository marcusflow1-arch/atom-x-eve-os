...
import { withAppearanceDefaults } from '@/components/onboarding/genesisAssets';
...
export default function GenesisWizard({ user, initialProfile, preview = false, onComplete }) {
  ...
  const submit = async (event) => {
    ...
    try {
      ...
      if (!response.data?.success) throw new Error(response.data?.error || 'Could not save setup.');
      if (preview) setSaved(response.data.profile);
      else onComplete(response.data.avatar);
    } catch (err) {
      setError(err.response?.data?.error || err.message || 'Could not save setup. Please retry.');
