...
export default function GlassPageFrame({ children, bottomContent, topContent, showTriggerTab = false, className = '', gameData, onGlobalSearch = null }) {
  ...
  return (
    <div className={`relative w-full h-full min-h-screen ${className}`}>
      ...
      >
        ...
        {bottomContent && (
          <div className="h-full w-full flex items-center justify-center px-6 py-2 gap-4">
            {bottomContent}
            {/* Global Search Bar — accessible from bottom header on all pages */}
            <button
              onClick={() => setSearchOpen(true)}
              className="px-4 py-2 rounded-lg bg-white/5 hover:bg-white/10 border border-white/10 hover:border-white/20 text-white/60 hover:text-white text-xs font-semibold transition-all flex items-center gap-2"
            >
