...
import { Sword, Sparkles, Box, Trash2, Eye, EyeOff, MousePointer } from 'lucide-react';
...
const TYPE_COLORS = {
  weapon: 'text-orange-400 border-orange-500/30 bg-orange-500/10',
  effect: 'text-cyan-400 border-cyan-500/30 bg-cyan-500/10',
  prop: 'text-slate-400 border-slate-500/30 bg-slate-500/10',
};

/**
 * Panel for managing the list of attached objects and editing the selected one's transform.
 */
  ...
  return (
    <div className="space-y-3 h-full flex flex-col">
      ...
      <div className="flex gap-1.5">
        <Button size="sm" onClick={() => onAdd('weapon')} className="flex-1 h-7 text-[9px] bg-orange-600/80 hover:bg-orange-600 gap-1">
          <Sword className="w-3 h-3" /> Weapon
        </Button>
        <Button size="sm" onClick={() => onAdd('effect')} className="flex-1 h-7 text-[9px] bg-cyan-600/80 hover:bg-cyan-600 gap-1">
          <Sparkles className="w-3 h-3" /> Effect
        </Button>
        <Button size="sm" onClick={() => onAdd('prop')} className="flex-1 h-7 text-[9px] bg-slate-600/80 hover:bg-slate-600 gap-1">
          <Box className="w-3 h-3" /> Prop
        </Button>
      </div>
        ...
        {objects.map(obj => {
          ...
          return (
            ...
            >
              ...
              <Badge className={`text-[8px] h-4 px-1.5 ${TYPE_COLORS[obj.type]}`}>{obj.type}</Badge>
              <button
      ...
      {selected ? (
        <div className="flex-1 overflow-y-auto space-y-3" style={{ scrollbarWidth: 'thin' }}>
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => onRemove(selected.id)} className="text-red-500 hover:text-red-400 p-1">
          ...
          {/* Bone */}
          <div>
            <label className="text-[9px] text-slate-500 uppercase mb-1 block">Bone</label>
            <select
              value={selected.bone || ''}
          ...
          <p className="text-[9px] text-cyan-500/60 italic mt-2">
            Click the object in the viewport to show gizmo handles for direct manipulation.
          </p>
        </div>
      ) : (
