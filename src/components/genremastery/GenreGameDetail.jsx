    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="relative flex-shrink-0 overflow-hidden">
        ...
        <div className="relative p-6 pb-0">
          ...
          <div className="grid grid-cols-4 gap-3 pb-5">
            {[
              {
                icon: Zap, color: 'text-cyan-400', gradBg: 'from-cyan-500/10 to-cyan-900/5',
                label: 'XP Earned', value: stats.earnedXP.toLocaleString(),
                sub: `${stats.totalXP.toLocaleString()} total`, pct: Math.round((stats.earnedXP / stats.totalXP) * 100)
              },
              {
                icon: Trophy, color: 'text-yellow-400', gradBg: 'from-yellow-500/10 to-yellow-900/5',
                label: 'Cards Unlocked', value: `${stats.earnedCards} / ${stats.totalCards}`,
                sub: 'achievement cards', pct: stats.totalCards > 0 ? Math.round((stats.earnedCards / stats.totalCards) * 100) : 0
              },
              {
                icon: Scroll, color: 'text-purple-400', gradBg: 'from-purple-500/10 to-purple-900/5',
                label: 'Quests Done', value: `${stats.completed} / ${stats.totalQuests}`,
                sub: 'completed', pct: Math.round((stats.completed / stats.totalQuests) * 100)
              },
              {
                icon: Users, color: 'text-blue-400', gradBg: 'from-blue-500/10 to-blue-900/5',
                label: 'Community', value: `${(game.communityCompletions || 0).toLocaleString()}+`,
                sub: 'player completions', pct: null
              },
            ].map((s, i) => (
              <motion.div
                key={i}
      ...
      <div className="px-6 py-3 flex-shrink-0 border-t border-white/5" style={{ background: 'rgba(8,12,18,0.6)' }}>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-white/30 text-xs">{filteredQuests.length} showing</span>
        </div>
              ...
              <div className="p-4 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <Badge className={`${quest.typeBg} ${quest.typeColor} border ${quest.typeBorder} text-[9px] px-1.5 py-0 h-5`}>
                      {quest.type}
                    </Badge>
                    <Badge className={`${diffBg(quest.difficulty)} ${diffColor(quest.difficulty)} text-[9px] px-1.5 py-0 h-5`}>
                      {quest.difficulty}
                    </Badge>
                    {quest.cardReward && (
