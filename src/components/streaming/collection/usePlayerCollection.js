...
export default function usePlayerCollection(user) {
  const client = useQueryClient();
  const query = useQuery({
    queryKey: ['stream-player-collection', user?.id, user?.unlocked_achievements, user?.achievement_rewards, user?.purchased_items, user?.current_activity?.gameId],
    enabled: Boolean(user?.id),
    staleTime: 60000,
    retry: 1,
    queryFn: async () => {
      const [ownedCards, userAchievements, achievements, tradingCards, games, libraryEntries] = await Promise.all([
        allRows(base44.entities.UserCard, { user_id: user.id }),
        allRows(base44.entities.UserAchievement, { user_id: user.id }),
        ...
        allRows(base44.entities.UserLibrary, { user_id: user.id }, ['id', 'game_id', 'game_title', 'game_genre', 'game_cover', 'last_played', 'play_time_minutes']).catch(() => []),
      ]);
