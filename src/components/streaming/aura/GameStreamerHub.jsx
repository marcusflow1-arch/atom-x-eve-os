...
import StreamerFocusView from './StreamerFocusView';
...
/**
 * Opened when a game is clicked on the Aura page.
 * Grid mode  → every streamer playing this game (Twitch-sized tiles).
 * Focus mode → 90% the selected streamer's page, 10% blurred rail of the others.
 */
        ...
        <div className="max-w-[1800px] mx-auto px-5 py-4 flex items-center gap-4 flex-wrap">
          {selected ? (
            <button
              ...
              className="px-3 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-white/80 text-xs font-bold inline-flex items-center gap-2"
            >
            ...
            <button
              onClick={onClose}
              className="w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 flex items-center justify-center text-white"
            >
              <X className="w-5 h-5" />
            </button>
          )}
          ...
          {selected && (
            <button
              onClick={onClose}
              className="ml-auto w-10 h-10 rounded-full bg-white/5 hover:bg-white/10 border border-white/10 flex items-center justify-center text-white"
            >
              <X className="w-5 h-5" />
            </button>
          )}
          ...
          {selected ? (
            <div key="focus" className="h-full">
              ...
              <div className="h-[90%] overflow-hidden">
                <StreamerFocusView streamer={selected} />
              </div>
              {/* 10% blurred blended rail */}
              <OtherStreamersRail
                streamers={others}
            ...
            >
              <div className="max-w-[1800px] mx-auto grid gap-x-5 gap-y-7 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
                {streamers.map((s) => (
                  <StreamerTile key={s.id} streamer={s} onClick={setSelected} />
                ))}
              </div>
            </motion.div>
          )}
