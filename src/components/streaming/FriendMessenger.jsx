  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col gap-1 max-w-xs ${isYou ? 'items-end' : 'items-start'}`}>
        ...
        {msg.type === 'video' && (
          <div
            className="rounded-xl overflow-hidden cursor-pointer hover:opacity-80 transition-opacity group"
          ...
          >
            ...
            <div className="p-2.5">
              <p className="text-xs font-bold text-white line-clamp-1">{msg.metadata.title}</p>
              <p className="text-[8px] text-white/50">Video Clip</p>
            </div>
          </div>
        )}
        ...
        {msg.type === 'guide' && (
          <div
            className="rounded-xl p-3 w-56 cursor-pointer hover:opacity-80 transition-opacity"
            style={{
          ...
          >
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-bold text-yellow-400">{msg.metadata.title}</p>
  ...
  return (
    <>
      ...
      <motion.div
        ...
        style={{
          right: '20px', top: '64px', bottom: '52px', width: '360px',
      ...
      >
        ...
        <div className="flex items-center justify-between px-4 py-3 flex-shrink-0"
          ...
          <div className="flex items-center gap-2.5 flex-1 min-w-0">
            <img src={friend.avatar} alt={friend.name} className="w-8 h-8 rounded-full object-cover ring-1 ring-white/20" />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-white truncate">{friend.name}</p>
              <p className={`text-[8px] ${friend.status === 'online' ? 'text-green-400' : 'text-white/50'}`}>
          ...
          <div className="flex items-center gap-1.5">
            <button
              ...
              className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${
                voiceActive ? 'bg-red-500/20 text-red-400' : 'text-white/40 hover:text-white'
              }`}
              style={{ border: voiceActive ? '1px solid rgba(239,68,68,0.3)' : '1px solid rgba(255,255,255,0.1)' }}
              title="Voice Call"
            >
        ...
        <div className="flex gap-1 px-3 pt-2 pb-2 flex-shrink-0"
          ...
          {[
            ...
            return (
              <button key={t.id}
                onClick={() => setTab(t.id)}
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[8px] font-bold uppercase tracking-widest transition-all flex-1"
                style={{
                  background: tab === t.id ? 'rgba(34,211,238,0.15)' : 'transparent',
                  border: tab === t.id ? '1px solid rgba(34,211,238,0.25)' : 'none',
                  color: tab === t.id ? '#67e8f9' : 'rgba(255,255,255,0.5)'
                }}>
            ...
            <div className="space-y-2">
              {messages.filter(m => m.type === 'video').map(msg => (
                <div key={msg.id}
                  ...
                  <div className="p-2.5">
                    <p className="text-xs font-bold text-white">{msg.metadata.title}</p>
