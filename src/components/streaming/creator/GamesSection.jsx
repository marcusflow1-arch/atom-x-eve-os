...
// The overlay lives inside the streaming box itself, inset 35px from its top, bottom and sides.
export default function GamesSection({ isEditMode, pinnedGames = [], onUpdateGames, onClose }) {
  ...
  const content = (
    <div
      ...
      role="dialog"
      aria-label="Games and achievements"
      className={fullscreen
        ? 'fixed inset-0 z-[100001] flex flex-col overflow-hidden rounded-none border-0 bg-slate-950/96 backdrop-blur-xl text-white'
        : 'absolute inset-[35px] z-[100000] flex flex-col overflow-hidden rounded-lg border border-white/[0.09] text-white shadow-[0_24px_70px_rgba(0,0,0,.55)]'}
      style={fullscreen ? undefined : {
        background: 'linear-gradient(180deg, rgba(2,6,23,.78), rgba(2,6,23,.86) 62%, rgba(2,6,23,.94))',
    ...
    >
      ...
      <div className="h-full w-full min-h-0 flex flex-col px-5 pt-3.5 pb-3">
        {/* Header */}
        <div className="flex items-center justify-between gap-4 shrink-0 pb-2.5 border-b border-white/[0.07]">
        ...
        {/* Vote banner */}
        {vote.session && (
          <div className="flex shrink-0 items-center gap-3 mt-2.5 px-3 py-2 rounded-md border border-cyan-300/25 bg-cyan-300/[0.06]">
        ...
        {/* Top: achievement cards for the selected game */}
        ...
        {/* Divider */}
        <div className="shrink-0 h-px bg-gradient-to-r from-transparent via-white/15 to-transparent my-2.5" />

        {/* Bottom: games rail, left to right */}
        <div className="shrink-0 flex flex-col min-h-0">
          ...
          <div className="h-[122px] shrink-0 overflow-x-auto overflow-y-hidden scrollbar-hide">
            <div className="h-full min-w-max flex items-stretch gap-3 pr-1">
              {filteredGames.map((game) => {
                ...
                return (
                  <div key={game.title} className={`relative group shrink-0 w-[210px] h-full border transition-all ${isSelected ? 'border-cyan-300/55 shadow-[0_0_22px_rgba(103,232,249,.16)]' : isMyVote ? 'border-cyan-300/45' : 'border-white/10 hover:border-white/25'}`}>
                    <button type="button" onClick={() => setSelectedGame(game)} className="block w-full h-full text-left" aria-label={`Show achievement cards for ${game.title}`}>
                      <div className="relative w-full h-[74px] overflow-hidden bg-slate-900">
