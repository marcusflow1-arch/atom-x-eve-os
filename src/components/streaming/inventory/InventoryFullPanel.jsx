...
import { X, Search, Mic, MicOff, Package, Gamepad2, Zap, Shield, User, Trees, Trophy, ChevronRight, ArrowLeftRight, DollarSign, Star, Trash2 } from 'lucide-react';
...
export default function InventoryFullPanel({ isOpen, onClose }) {
  ...
  // Generate all inventory data once
  const allInventory = useMemo(() => {
    const map = {};
    libraryGames.forEach(g => {
      map[g.id] = generateInventoryForGame(g);
    });
    return map;
  }, []);
  ...
  // Filtered items for selected game
    ...
    >
      ...
      <div className="sticky top-0 bg-[#0c1018]/95 backdrop-blur-xl z-10 p-6 pb-4 border-b border-white/5">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-2xl font-bold text-white">
                {selectedGame ? (selectedGame.title || selectedGame.name) : 'Full Inventory'}
              </h2>
              <p className="text-sm text-white/40">
                {selectedGame ? `${gameItems.length} items` : `${totalItems} items across ${libraryGames.length} games`}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            {selectedGame && (
              <button onClick={() => { setSelectedGame(null); setSelectedItem(null); }} className="px-3 py-1.5 rounded-lg bg-white/5 border border-white/10 text-xs text-white/60 hover:text-white hover:bg-white/10 transition-all flex items-center gap-1.5">
                <ChevronRight className="w-3 h-3 rotate-180" /> All Games
              </button>
            )}
        ...
        <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-full px-4 py-2 focus-within:border-white/20 transition-all">
          ...
          <input
            ...
            placeholder={isListening ? 'Listening...' : selectedGame ? 'Search items...' : 'Search games or items...'}
      ...
      <div className="flex-1 overflow-y-auto p-6">
        <AnimatePresence mode="wait">
          {!selectedGame ? (
            /* GAME LIST VIEW */
            <motion.div key="games" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              {filteredGames.map((game, i) => {
                ...
                return (
                  <motion.div
                    ...
                    onClick={() => { setSelectedGame(game); setSelectedItem(null); }}
                    className="group relative aspect-[3/4] rounded-xl overflow-hidden bg-white/5 border border-white/10 cursor-pointer hover:border-amber-400/50 hover:shadow-[0_0_20px_rgba(245,158,11,0.2)] transition-all"
                  >
            ...
            /* GAME INVENTORY VIEW */
            <motion.div key="items" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
              ...
              {gameItems.length === 0 && (
                <div className="col-span-full py-16 text-center text-white/30">No items match your search.</div>
              )}
            </motion.div>
          )}
        </AnimatePresence>
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className={`relative h-56 flex-shrink-0 flex items-center justify-center ${cfg.bg}`}>
          ...
          <div
            className={`w-36 aspect-[2.5/3.5] rounded-xl border-2 ${cfg.border} flex items-center justify-center relative overflow-hidden`}
            style={{
