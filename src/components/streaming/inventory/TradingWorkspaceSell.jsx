...
export default function TradingWorkspaceSell({ item, marketPrice }) {
  ...
  const fee = sellMode === 'fixed' && sellPrice ? Math.floor(parseInt(sellPrice) * 0.05) : 0;
  const net = sellMode === 'fixed' && sellPrice ? parseInt(sellPrice) - fee : 0;

  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="p-5 space-y-5">

      {/* Toggle: Fixed Price vs Auction */}
      <div className="flex gap-1 p-1 rounded-xl bg-white/[0.04] border border-white/5">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg text-xs font-bold uppercase tracking-wider transition-all ${
            sellMode === 'fixed'
              ? 'bg-green-500/20 border border-green-500/30 text-green-300 shadow-lg shadow-green-500/10'
              : 'text-white/40 hover:text-white/60 border border-transparent'
          }`}
        >
          <DollarSign className="w-3.5 h-3.5" /> Fixed Price
        </button>
        <button
      ...
      {/* ─── FIXED PRICE MODE ─── */}
      {sellMode === 'fixed' && (
        <motion.div key="fixed" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      {/* ─── AUCTION MODE ─── */}
      {sellMode === 'auction' && (
        <motion.div key="auction" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
          <div className="bg-purple-900/15 border border-purple-500/15 rounded-xl p-4 space-y-4">
            ...
            <div className="bg-white/[0.03] border border-white/5 rounded-lg p-3 space-y-1.5">
              ...
              <div className="flex justify-between text-[10px]">
                <span className="text-white/40">Reserve Met Guarantee</span>
