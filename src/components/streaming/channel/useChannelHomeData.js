...
export default function useChannelHomeData(channelId, profileId) {
  ...
  useEffect(() => {
    ...
    for (const name of ['StreamLayoutConfig', 'AuraStreamSchedule', 'Stream', 'AuraStream', 'AuraSponsor']) { try { cleanups.push(base44.entities[name].subscribe(refresh)); } catch { /* Timed refresh remains available. */ } }
