...
import { Search, Maximize2, Minimize2, Radio, X, Gamepad2, ArrowLeft } from 'lucide-react';
...
export default function AuraStreamBrowserOverlay() {
  ...
  const filtered = useMemo(() => games.filter(g => {
    const genreOk = selectedGenre === 'All' || g.genre === selectedGenre;
    const queryOk = !query.trim() || String(g.title || '').toLowerCase().includes(query.toLowerCase());
    return genreOk && queryOk;
  }), [games, selectedGenre, query]);

  const chooseGame = game => {
    // IMPORTANT: selecting a game changes the CONTENT OF THIS OVERLAY only.
    // It must not close the overlay and must not replace/navigate the Aura page.
  ...
  return (
    <AnimatePresence>
      {open && (
        <motion.div className="fixed inset-0 z-[10000] pointer-events-none" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
          <motion.section
          ...
          >
            {selectedGame ? (
              <div className="h-full w-full flex flex-col">
                <header className="h-14 shrink-0 flex items-center gap-3 px-5 border-b border-white/10 bg-black/10">
                  <button onClick={() => setSelectedGame(null)} className="p-2 text-white/60 hover:text-white" title="Back to streaming games"><ArrowLeft size={17}/></button>
                  <Radio className="w-4 h-4 text-purple-300" />
                  <span className="text-xs font-bold uppercase tracking-[.2em]">Live Streamers · {selectedGame.title || 'Selected Game'}</span>
                  <div className="ml-auto flex items-center gap-2">
                    ...
                    <button onClick={() => { setSelectedGame(null); setOpen(false); }} className="p-2 text-white/60 hover:text-white" title="Close"><X size={17}/></button>
                  </div>
                </header>
