...
export function resolveMedia(value, hostname = 'localhost', preview = false, loop = false) {
  ...
  try {
    ...
    if (['youtube.com', 'youtu.be', 'youtube-nocookie.com'].includes(host)) {
      ...
      return { type: 'embed', url: `https://www.youtube-nocookie.com/embed/${id}?autoplay=1&mute=1&playsinline=1&controls=${preview ? 0 : 1}&rel=0${preview || loop ? `&loop=1&playlist=${id}` : ''}` };
    }
...
export default function StreamMedia({ url, poster, title, preview = false, active = true, offline = false, loop = false }) {
  ...
  const media = useMemo(() => resolveMedia(url, window.location.hostname, preview, loop), [url, preview, loop]);
  ...
  return <div className={`console-media ${preview ? 'is-preview' : ''}`}>
    ...
    {media && !failed && (media.type === 'embed'
      ...
      : <video ref={videoRef} poster={poster} controls={!preview} autoPlay={active} muted={muted} playsInline loop={(preview || loop) && media.type === 'video'} preload={preview ? 'none' : 'metadata'} aria-label={title || 'Live stream'} onError={() => setFailed(true)} onVolumeChange={(event) => setMuted(event.currentTarget.muted)} />)}
