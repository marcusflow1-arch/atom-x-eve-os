          ...
          <div className="w-[260px] hidden lg:block flex-shrink-0 sticky top-20 self-start">
            ...
            <div className="space-y-2 pr-1 custom-scrollbar overflow-y-auto max-h-[calc(100vh-6rem)]">
              {genres.map((g) => (
                ...
                >
                  {g === "All" ? (
                    <LayoutGrid className={`w-5 h-5 flex-shrink-0 ${activeGenre === g ? "text-cyan-400" : "text-white/60"}`} />
                  ) : (
                    <Gamepad2 className={`w-5 h-5 flex-shrink-0 ${activeGenre === g ? "text-cyan-400" : "text-white/60"}`} />
                  )}
