...
import { inventoryData, itemData } from '../components/profile/mockData';
...
export default function LunaTemplate() {
  ...
  // NOTE: Model3D.list() is shared with the scene loader below via window.__model3dCache to avoid duplicate API calls
  useEffect(() => {
    let cancelled = false;
    const load = async () => {
      try {
        // Reuse cached model list if scene loader already fetched it
        const models = window.__model3dCache || await base44.entities.Model3D.list().then(r => { window.__model3dCache = r; return r; });
        const lara = models.find(m => {
          const name = (m.name || '').toLowerCase();
          return name === 'lara' || (name.includes('lara') && !name.includes('xbot') && !name.includes('ybot'));
        });
        const url = (lara?.file_url) || 'https://raw.githubusercontent.com/mrdoob/three.js/master/examples/models/gltf/Xbot.glb';
        if (!cancelled) setModelUrl(url);
      } catch (e) {
        if (!cancelled) setModelUrl('https://raw.githubusercontent.com/mrdoob/three.js/master/examples/models/gltf/Xbot.glb');
      }
    };
    if (!modelUrl) load();
    ...
    const loadScene = async () => {
      ...
      try {
        ...
        if (layouts.length > 0) {
          ...
          // 3. Fallback: load Model3D list once (reuse cache if already fetched)
          const models = window.__model3dCache || await base44.entities.Model3D.list().then(r => { window.__model3dCache = r; return r; });
  ...
  useEffect(() => {
    const onKey = async (e) => {
      ...
      if (key === 'escape') {
        if (avatarFocusMode) { setAvatarFocusMode(false); return; }
  ...
  }, [showForumOverlay, showAvatarProgression, showSkillTreeBlankUI, avatarFocusMode, navigate, showLibraryLanding, librarySelection, showDevSpotlight, hideUI, selectedFocusGame, longPressGame, showFriendsHub]);
  ...
  return (
    ...
    <div className="h-screen w-full flex relative overflow-hidden text-white font-sans selection:bg-cyan-500/30" style={{ backgroundColor: '#070a11' }}>
      ...
      <AnimatePresence>
        {!uiVisible && !showConsoleMode && !avatarFocusMode && !activeSubTab && homeSection === 'avatar' && !showLibraryLanding &&
                <motion.div
...
{!showAvatarProgression &&
      ...
      {!avatarFocusMode && !clickedSlot && !showConsoleMode && !showAchievements && !activeSubTab &&
