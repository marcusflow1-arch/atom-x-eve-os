...
export default function Library({ onSwitchToStore, onSwitchToAchievements }) {
  ...
  const [viewMode, setViewMode] = useState('line'); // 'grid' | 'list' | 'line'
  ...
  return (
    ...
    >
      ...
      <div className="relative z-10 px-8 py-8">
            ...
            <div className="flex items-center gap-1 p-1 rounded-xl bg-white/5 border border-white/10">
              <button
                ...
                className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                  viewMode === 'line' ? 'bg-white/10 text-white' : 'text-white/40 hover:text-white'
                }`}
                title="Line View"
              >
        ...
        {viewMode === 'line' ? (
          /* LINE VIEW - Games scroll vertically on far left with fading cover, options appear to the right */
          <LineViewLayout 
            games={filteredGames}
            ...
            handleStreamGame={handleStreamGame}
          />
        ) : viewMode === 'grid' ? (
