...
// --- Library Grid View Component (Steam-style with Left Sidebar, Center Content, Right Stats) ---
const LibraryGridView = ({ games, onLaunchGame, onStreamGame, onSwitchToAchievements, onSwitchToList, searchTerm, setSearchTerm }) => {
      ...
      <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ background: 'transparent' }}>
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-1">
            <button className="px-4 py-1.5 rounded-full bg-blue-500 text-white text-sm font-medium">Games</button>
            <button className="px-4 py-1.5 rounded-full text-white/60 hover:text-white text-sm font-medium transition-all">Marketplace</button>
            <button className="px-4 py-1.5 rounded-full text-white/60 hover:text-white text-sm font-medium transition-all">Trading Post</button>
          </div>
        </div>
        ...
        >
          ...
          <div className="p-3 border-b border-white/10">
            <button
              ...
              className="flex items-center gap-2 px-3 py-2 rounded-lg bg-yellow-500/10 border border-yellow-500/30 text-yellow-400 hover:bg-yellow-500/20 transition-all w-full"
            >
          ...
          <div className="flex-1 overflow-y-auto p-3">
            ...
            <div>
              <button 
                ...
                className="flex items-center justify-between w-full text-white/50 text-xs font-semibold uppercase tracking-wider mb-2"
              >
                    ...
                    <div key={genre}>
                      ...
                      <div className="pl-3 space-y-0.5">
                        {gamesByGenre[genre].map((game) => (
                          ...
                          >
                            <div className="w-7 h-7 rounded overflow-hidden flex-shrink-0 bg-black/30">
          ...
          {selectedGame ? (
            <>
              ...
              <div className="flex gap-4 p-4 flex-shrink-0">
                {/* Game Cover */}
                <div className="w-28 h-36 rounded-lg overflow-hidden border border-white/10 flex-shrink-0">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2">
                    <Button onClick={() => onLaunchGame(selectedGame)} className="bg-white text-black hover:bg-white/90 font-semibold px-5 h-9">
                      <Play className="w-4 h-4 mr-1.5 fill-current" /> Play
                    </Button>
                    <Button variant="outline" onClick={() => onStreamGame(selectedGame)} className="border-white/20 hover:bg-white/10 text-white h-9 px-4">
              ...
              {/* Tabs */}
              <div className="flex items-center gap-4 px-4 border-b border-white/10 flex-shrink-0">
                {tabs.map((tab) => (
                  <button
                    key={tab}
                    onClick={() => setActiveTab(tab.toLowerCase().replace(' ', '_'))}
                    className={`pb-2.5 text-xs font-medium transition-all relative uppercase tracking-wide ${
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {activeTab === 'overview' && (
                  <div className="space-y-4">
                    ...
                    <div className="relative aspect-video rounded-xl overflow-hidden bg-black/50 border border-white/10 max-w-xl">
                      <img src={selectedGame.banner || selectedGame.cover_image || selectedGame.cover} alt="trailer" className="w-full h-full object-cover opacity-50" />
                ...
                {activeTab === 'discussion' && (
                  <div className="space-y-3">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Button size="sm" className="bg-blue-500/20 text-blue-300 hover:bg-blue-500/30 border border-blue-500/30 text-xs h-8">
                        <MessageSquare className="w-3 h-3 mr-1" /> New Post
                      </Button>
                    </div>
                    {[
                      ...
                      <div key={i} className="p-3 rounded-lg border border-white/10 hover:border-white/20 cursor-pointer transition-all" style={{ background: 'rgba(30, 40, 60, 0.5)' }}>
                ...
                {activeTab === 'streamers' && (
                  <div className="space-y-3">
                    <h3 className="text-white font-semibold mb-3">Live Streamers</h3>
                    {[
                      { name: 'ProGamer2024', viewers: 1234, title: 'Speedrunning!', avatar: 'P' },
                      { name: 'CasualPlayer', viewers: 567, title: 'Chill playthrough', avatar: 'C' },
                    ].map((streamer, i) => (
                      <div key={i} className="flex items-center gap-3 p-3 rounded-lg border border-white/10" style={{ background: 'rgba(30, 40, 60, 0.5)' }}>
                        <div className="w-10 h-10 rounded-full bg-red-500 flex items-center justify-center text-white font-bold text-sm">{streamer.avatar}</div>
                        <div className="flex-1">
                          <h4 className="text-white font-medium text-sm">{streamer.name}</h4>
                          <p className="text-white/50 text-xs">{streamer.title}</p>
                        </div>
                ...
                {activeTab === 'guide' && (
                  <div className="space-y-3">
                    <h3 className="text-white font-semibold mb-3">Game Guides</h3>
                    {[
                      { title: 'Complete Walkthrough', type: 'Walkthrough', rating: 4.9 },
                      { title: 'All Collectibles Guide', type: 'Collectibles', rating: 4.8 },
                    ].map((guide, i) => (
                      <div key={i} className="p-3 rounded-lg border border-white/10" style={{ background: 'rgba(30, 40, 60, 0.5)' }}>
                        <div className="flex items-center justify-between mb-1">
                          <h4 className="text-white font-medium text-sm">{guide.title}</h4>
                          <Badge className="bg-blue-500/20 text-blue-300 border-blue-500/30 text-[10px]">{guide.type}</Badge>
                        </div>
                        <div className="flex items-center gap-1 text-xs text-white/40">
                          <Star className="w-3 h-3 text-yellow-400" /> {guide.rating}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {activeTab === 'support' && (
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="p-3 rounded-lg border border-white/10 text-center cursor-pointer hover:border-white/20" style={{ background: 'rgba(30, 40, 60, 0.5)' }}>
                      ...
                      <div className="p-3 rounded-lg border border-white/10 text-center cursor-pointer hover:border-white/20" style={{ background: 'rgba(30, 40, 60, 0.5)' }}>
                        ...
                        <h4 className="text-white font-medium text-sm">FAQ</h4>
                      </div>
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-white/50 text-xs">53%</span>
                      </div>
                    </div>
                    {[
                      { name: 'First Steps', icon: '👣', unlocked: true, rarity: 'Common' },
                      { name: 'Dragon Slayer', icon: '🐉', unlocked: true, rarity: 'Rare' },
                      { name: 'Legendary Hero', icon: '👑', unlocked: false, rarity: 'Legendary' },
                    ].map((achievement, i) => (
                      <div key={i} className={`flex items-center gap-3 p-3 rounded-lg border ${achievement.unlocked ? 'border-white/20' : 'border-white/5 opacity-50'}`} style={{ background: 'rgba(30, 40, 60, 0.5)' }}>
                        <div className="text-2xl">{achievement.icon}</div>
                        <div className="flex-1">
                          <h4 className="text-white font-medium text-sm">{achievement.name}</h4>
                        </div>
                        <Badge className={achievement.rarity === 'Legendary' ? 'bg-orange-500/20 text-orange-300 border-orange-500/30 text-[10px]' : achievement.rarity === 'Rare' ? 'bg-blue-500/20 text-blue-300 border-blue-500/30 text-[10px]' : 'bg-slate-500/20 text-slate-300 border-slate-500/30 text-[10px]'}>
                          {achievement.rarity}
                        </Badge>
                      </div>
                    ))}
                ...
                {activeTab === 'streamer_affiliate' && (
                  <div className="space-y-3">
                    <h3 className="text-white font-semibold mb-3">Streamer Affiliate Program</h3>
                    <div className="p-4 rounded-lg border border-purple-500/30" style={{ background: 'rgba(147, 51, 234, 0.1)' }}>
                      <h4 className="text-white font-bold mb-1">Become an Affiliate</h4>
                      <p className="text-white/60 text-sm mb-3">Earn rewards by streaming this game.</p>
                      <Button className="bg-purple-500 hover:bg-purple-600 text-white text-sm h-8">Apply Now</Button>
                    </div>
                  </div>
                )}
              </div>
        ...
        <div className="w-56 flex-shrink-0 border-l border-white/10 flex flex-col" style={{ background: 'rgba(20, 30, 45, 0.3)' }}>
          ...
          <div className="p-3 border-b border-white/10 space-y-3">
            <div className="relative">
              ...
              <input
                ...
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                className="w-full bg-white/5 border border-white/10 rounded-lg pl-10 pr-8 py-2 text-sm text-white placeholder:text-white/30 focus:outline-none focus:border-white/20"
              />
            ...
            <div className="flex items-center justify-end border border-white/10 rounded-lg overflow-hidden">
              <button className="p-2 bg-white/10 text-white">
                <LayoutGrid className="w-4 h-4" />
              </button>
              <button onClick={onSwitchToList} className="p-2 text-white/40 hover:text-white/70">
          ...
          {selectedGame && (
            <div className="flex-1 overflow-y-auto p-3 space-y-3">
              <div className="p-3 rounded-lg border border-white/10" style={{ background: 'rgba(30, 40, 60, 0.5)' }}>
                <h4 className="text-white/50 text-xs font-bold uppercase tracking-wider mb-2">Game Stats</h4>
              ...
              <div className="p-3 rounded-lg border border-white/10" style={{ background: 'rgba(30, 40, 60, 0.5)' }}>
                <h4 className="text-white/50 text-xs font-bold uppercase tracking-wider mb-2">Friends Playing</h4>
                <div className="flex items-center gap-1.5">
                  {['A', 'B', 'C'].map((letter, i) => (
                    <div key={i} className="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold" style={{ background: ['#3b82f6', '#22c55e', '#f59e0b'][i] }}>
                      {letter}
                    </div>
                  ))}
                </div>
