...
export default function StreamingHome() {
    ...
    <div className="h-full min-h-0 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 min-h-0 flex overflow-hidden">
        ...
        <section className="w-4/5 min-w-0 pl-5 md:pl-7 pt-4 flex flex-col min-h-0 overflow-hidden">
          ...
          <div className="flex-1 min-h-0 overflow-y-auto pr-2 scrollbar-hide"><div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 pb-4">{cardItems.map((name, index) => { const rarity = index % 5 === 0 ? 'Legendary' : index % 3 === 0 ? 'Epic' : index % 2 === 0 ? 'Rare' : 'Common'; const style = RARITY_STYLES[rarity]; const key = `${selectedCardGame}-${name}`; const hovered = hoveredCard === key; return <button key={key} type="button" onClick={() => setSelectedCard({ name, id: key, game: selectedCardGame, rarity })} onMouseMove={(event) => handleCardPointerMove(event, key)} onMouseLeave={handleCardPointerLeave} className="relative aspect-[0.72] min-h-[200px] text-left outline-none" style={{ perspective: '900px' }}><motion.div animate={{ rotateX: hovered ? cardTilt.x : 0, rotateY: hovered ? cardTilt.y : 0, y: hovered ? -5 : 0, scale: hovered ? 1.025 : 1 }} transition={{ type: 'spring', stiffness: 260, damping: 24 }} className="absolute inset-0 overflow-hidden border bg-slate-950 shadow-[0_18px_42px_rgba(0,0,0,.42)]" style={{ borderColor: hovered ? style.edge : 'rgba(255,255,255,.12)', boxShadow: hovered ? `0 18px 46px ${style.glow}, 0 0 28px ${style.glow}` : undefined }}><div className="absolute -inset-8 opacity-80" style={{ background: `radial-gradient(circle at 72% 20%, ${style.glow}, transparent 36%), linear-gradient(145deg, rgba(255,255,255,.08), transparent 28%, rgba(124,58,237,.12) 75%, rgba(15,23,42,.95))` }} /><div className="absolute inset-x-0 top-0 h-[52%] overflow-hidden"><img src={CARD_GAMES.find((game) => game.id === selectedCardGame)?.image} alt="" className="w-full h-full object-cover opacity-70 mix-blend-screen" /><div className="absolute inset-0 bg-gradient-to-b from-transparent via-slate-950/10 to-slate-950" /></div><div className="absolute top-3 left-3 right-3 flex items-center justify-between gap-2"><span className="px-1.5 py-0.5 text-[7px] uppercase tracking-[0.16em] font-bold border bg-black/45" style={{ color: style.accent, borderColor: style.edge }}>{rarity}</span><Sparkles className="w-3.5 h-3.5" style={{ color: style.accent }} /></div><div className="absolute left-3 right-3 bottom-3"><div className="text-[8px] uppercase tracking-[0.18em] text-white/35 mb-1">{CARD_GAMES.find((game) => game.id === selectedCardGame)?.name}</div><div className="text-sm font-extrabold text-white leading-tight">{name}</div><div className="mt-2 h-px bg-gradient-to-r from-white/30 via-white/10 to-transparent" /><div className="text-[8px] uppercase tracking-wider text-white/30 mt-2">Collectible achievement</div></div>{hovered && <motion.div initial={{ x: '-120%', opacity: 0 }} animate={{ x: '120%', opacity: [0, .65, 0] }} transition={{ duration: .9, ease: 'easeInOut' }} className="absolute top-0 bottom-0 w-8 -skew-x-12 bg-gradient-to-r from-transparent via-white/80 to-transparent blur-[2px] pointer-events-none" />}</motion.div></button>; })}</div></div>
        </section>
      </div>
    </div>
  );
  ...
  const renderOverlayContent = () => {
    ...
    if (activeTab === 'gallery') return <GallerySection isEditMode={isEditMode} galleryImages={galleryImages} onUpdateImages={(imgs) => updateEditLayout('gallery_images', imgs)} onClose={closeOverlay} fullscreen={overlayFullscreen} />;
