...
import {
  ...
  Home, ShoppingBag, Library, Trophy, Layers, Hammer, Bot, ArrowLeftRight,
  Gavel, LogIn, LogOut, Swords, Rocket, Plus, Ghost, Footprints, Gem, Check, Truck, Award, Users
} from 'lucide-react';
...
export default function Marketplace() {
  ...
  return (
      ...
      <header className="sticky top-0 z-50 bg-slate-900/95 backdrop-blur-xl border-b border-white/10">
        <div className="max-w-[1400px] mx-auto px-4 h-14 flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            <button onClick={() => setDrawerOpen(true)} className="w-9 h-9 rounded-lg bg-white/10 hover:bg-white/20 flex items-center justify-center">
            ...
            <span className="text-white font-bold text-lg hidden sm:block">Atom X Eve Market</span>
          </div>

          {/* Search */}
          <div className="flex-1 max-w-2xl">
            <div className="flex bg-white/10 rounded-lg overflow-hidden border border-white/10 focus-within:border-orange-500/50">
              <select
                value={filters.category}
                onChange={(e) => setFilters(prev => ({ ...prev, category: e.target.value }))}
                className="bg-slate-800 text-white/70 text-sm px-3 py-2 border-r border-white/10 focus:outline-none"
              >
              ...
              />
              <button className="bg-orange-500 hover:bg-orange-600 px-4 text-white">
          ...
          <div className="flex items-center gap-3">
            <div className="hidden md:flex items-center gap-2 text-sm">
              ...
              <span className="text-white font-bold">24,500 AGP</span>
            </div>
            <Button variant="ghost" className="relative p-2 hover:bg-white/10">
        ...
        <div className="bg-slate-800/50 border-t border-white/5">
          <div className="max-w-[1400px] mx-auto px-4 h-10 flex items-center gap-6 text-sm overflow-x-auto scrollbar-hide">
            {CATEGORIES.slice(1).map(cat => (
              <button
                key={cat.id}
                onClick={() => setFilters(prev => ({ ...prev, category: cat.id }))}
                className={`whitespace-nowrap flex items-center gap-1.5 ${filters.category === cat.id ? 'text-orange-400' : 'text-white/70 hover:text-white'}`}
              >
      ...
      <AnimatePresence>
        {drawerOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black/60 z-50" onClick={() => setDrawerOpen(false)} />
            <motion.div initial={{ x: -300 }} animate={{ x: 0 }} exit={{ x: -300 }} className="fixed top-0 left-0 bottom-0 w-72 bg-slate-900 border-r border-white/10 z-50 p-4">
              <div className="flex items-center justify-between mb-6">
                <span className="text-white font-bold">Menu</span>
                <button onClick={() => setDrawerOpen(false)}><X className="w-5 h-5 text-white/50" /></button>
              </div>
              {allNavItems.map(item => (
                <Link key={item.name} to={item.path} onClick={() => setDrawerOpen(false)} className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-white/70 hover:text-white hover:bg-white/10">
