...
Deno.serve(async (req) => {
  try {
    ...
    switch (action) {
      case 'list': {
        // List files created by this app or opened by user via picker
        ...
        const params = new URLSearchParams({
          q,
          fields: 'nextPageToken,files(id,name,mimeType,size,modifiedTime,createdTime,iconLink,thumbnailLink,webViewLink,webContentLink,shared,owners,sharingUser,permissions,parents)',
          pageSize: '50',
      ...
      case 'get': {
        const { fileId } = body;
        const resp = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?fields=id,name,mimeType,size,modifiedTime,createdTime,webViewLink,webContentLink,shared,owners,permissions,description`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
        const data = await resp.json();
      ...
      case 'upload': {
        ...
        // Simple upload for text/small content
        ...
        const resp = await fetch('https://www.googleapis.com/upload/drive/v3/files?uploadType=multipart&fields=id,name,mimeType,webViewLink,webContentLink', {
          method: 'POST',
          headers: {
      ...
      case 'createFolder': {
        ...
        const resp = await fetch('https://www.googleapis.com/drive/v3/files?fields=id,name,mimeType,webViewLink', {
          method: 'POST',
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      case 'rename': {
        ...
        const resp = await fetch(`https://www.googleapis.com/drive/v3/files/${fileId}?fields=id,name,mimeType,webViewLink`, {
          method: 'PATCH',
          headers: {
            Authorization: `Bearer ${accessToken}`,
            'Content-Type': 'application/json',
