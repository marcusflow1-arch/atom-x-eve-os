...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const specPattern = new RegExp('"spec":"(https?:\\\\/\\\\/[^\"]*storyboard[^\"]*)"');
    const spec=html.match(specPattern)?.[1]?.replace(/\\u0026/g,'&').replace(/\\\//g,'/')||'';
    ...
    let frameCount=0;
    if(spec){const storyboardUrlPattern=new RegExp('https?:\\\\/\\\\/[^&"\\\\]+','g');const urls=[...spec.matchAll(storyboardUrlPattern)].map(m=>m[0].replace(/\\u0026/g,'&')).filter((u,i,a)=>a.indexOf(u)===i).slice(0,100);const frames=urls.map((u,i)=>({video_analysis_id:va.id,frame_index:i,timestamp_seconds:i,image_url:u,thumbnail_url:u,frame_summary:`Real chronological storyboard source ${i+1} for ${title}`,notes:`Visual evidence for ${knowledgeType}.`,is_representative:i===0}));try{if(frames.length){await base44.asServiceRole.entities.VideoFrame.bulkCreate(frames);frameCount=frames.length;}}catch(e){console.log('Frame persistence warning',e?.message||e);}}
