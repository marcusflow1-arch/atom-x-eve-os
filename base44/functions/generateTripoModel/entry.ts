...
async function createTask(input: any) {
  ...
  let body: any = {
    prompt: input.prompt || 'A production-ready game asset',
    model,
    texture: input.texture !== false,
    pbr: input.pbr !== false,
    texture_quality: input.textureQuality || 'detailed',
    geometry_quality: input.geometryQuality || 'detailed',
    export_uv: input.exportUv !== false,
  };
  ...
  if (type === 'image_to_model') {
    ...
    body = {
      input: input.sourceImageUrl,
      model,
      texture: input.texture !== false,
      pbr: input.pbr !== false,
      texture_quality: input.textureQuality || 'detailed',
      export_uv: input.exportUv !== false,
      enable_image_autofix: input.autoFix !== false,
  ...
  const result = await tripoFetch(endpoint, {
    method: 'POST',
...
async function getTask(taskId: string) {
  return tripoFetch(`/tasks/${encodeURIComponent(taskId)}`, { method: 'GET', headers: { 'Content-Type': undefined as any } });
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === 'status') {
      ...
      const task = (await getTask(body.taskId))?.data;
    ...
    if (task.status === 'success') {
      ...
      await base44.asServiceRole.entities.TripoModel.update(recordId, {
        ...
        metadata: {
          ...(record.metadata || {}),
          credits_consumed: task.credits_consumed ?? null,
          created_at: task.created_at || null,
          completed_at: task.completed_at || null,
          note: 'Tripo model URLs expire; download/import immediately when available.',
        },
      });
    ...
    if (task.timeout) {
      await base44.asServiceRole.entities.TripoModel.update(recordId, {
        ...
        error_message: 'Generation is still running. Call this function with action=status and the taskId to check progress.',
      });
    ...
    await base44.asServiceRole.entities.TripoModel.update(recordId, {
      ...
      progress: Number(task.progress || 0),
      error_message: task.error_message || `Tripo task ended with status ${task.status}`,
    });
    return json({ success: false, id: recordId, task }, 502);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    if (recordId && base44) {
      await base44.asServiceRole.entities.TripoModel.update(recordId, { status: 'failed', error_message: message }).catch(() => {});
    }
